...
Func &Func::coordinates(Var x, Var y, Var c, DeviceAPI device_api) {
    invalidate_cache();
    ...
    user_assert(constant_bounds)
        << "The color channel for coordinates loops must have constant bounds, e.g., .bound(c, 0, 3).\n";
...
Func &Func::glsl(Var x, Var y, Var c) {
    return coordinates(x, y, c, DeviceAPI::GLSL);
}

Func &Func::rs(Var x, Var y, Var c) {
    return coordinates(x, y, c, DeviceAPI::RS);
}
